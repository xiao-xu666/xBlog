...
xBlog博客，一个**一个基于MVC架构的SSM前后端分离博客系统**。**Web** 端使用 **Vue** + **ElementUi** ，后端使用 **Spring** + **SpringBoot** + **Mybatis-plus**进行开发，使用**redis**+**mailSender** 做登录验证和邮箱注册，使用 **Mybatis-plus** 作为持久化操作核心，使用 **Git**完成博客的持续集成，使用 **MySQL** 收集博客数据，文件支持**上传本地**、**阿里云OOS** .
